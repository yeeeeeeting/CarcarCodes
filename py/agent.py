...
class SimpleAgent:
    ...
    @classmethod
    def activation_msg(cls):
        return cls.CMD_END + 'a' + cls.CMD_END;
    ...
    def check_no_response(self):
        ...
        if not self.activated and self.last_msg_time - self.last_msg_time > 2.0:
            return self.repeat
        else:
            return None
    ...
    def on_message(self, msg: str):
        ...
        if msg[0] == 'v':
            if self.repeat == self.activation_msg():
                self.activated = True
        ...
        if msg[0] == 'A':
            ...
            if msg.endswith("AT+RESET"):
                self.counter = 0
                time.sleep(3.5)
                self.repeat = self.activation_msg()
                return self.repeat
            else:
                return None
        ...
        if msg[0] == 'q':
            # self.update_msg_time()
            result = self.actions[self.counter]
            self.counter += 1
    ...
    def parse(msg: str):
        ...
        for c in msg:
            c = ord(c)
            code += SimpleAgent.hexChar((c & 0xf0) >> 4)
            code += SimpleAgent.hexChar((c & 0x0f))
        return code
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print("$ bytes =", bytes)
        print("$ s =", s)
        print("$ result =", SimpleAgent.parse(s))
